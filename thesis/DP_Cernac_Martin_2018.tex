...
\section{ICMP tunelování}

Tunelování dat pomocí \texttt{ICMP} je technicky možné díky zprávám typu \texttt{0} a \texttt{8} (\textit{echo reply}, resp. \textit{echo request}), pro které je specifikována proměnlivá délka zpráv. Zprávy těchto typů jsou používány zejména nástrojem \texttt{ping}. Zdrojová stanice vyšle na cílovou stanici zprávu typu \texttt{echo request} a cílová stanice odpoví zprávou \texttt{echo reply}. Strukturu použitých \texttt{ICMP} zpráv znázorňuje diagram\ref{diag:icmp-packet}
    ...
    Protože \texttt{ICMP} pakety nemají na rozdíl od \texttt{UDP} a \texttt{TCP} paketů zdrojový a cílový port, síťové prvky jako například \texttt{NAT} musejí používat jiné metody pro zacházení s \texttt{ICMP} pakety. Dle \cite{rfc792}[RFC 792] a  \cite{rfc3022}[RFC 3022] lze pro tento účel u zpráv \texttt{echo request} a \texttt{echo reply} pole \textit{Identifikátor}, tedy \texttt{16} bitovou položku pro rozpoznání souvisejících zpráv. Specifikace se nezmiňuje o délce platnosti \textit{identifikátoru}. Díky tomu je možno udržovat komunikaci i pokud je jeden z účastníků omezen \texttt{NAT}, protože je možné klientovi \uv{za NATem} zaslat více paketů se shodnými hodnotami \textit{sekvenční číslo} a \textit{identifikátor}.
    
    Aby obě strany tunelu byly schopny identifikovat tok dat tunelu a rozeznat ho od jiných \texttt{ICMP} zpráv, je nutné \uv{tunelové} \texttt{ICMP} zprávy označit. Právě proto začíná každá taková \texttt{ICMP} zpráva stejnou sekvencí čtyř bajtů, která označuje \texttt{ICMP} zprávy, nesoucí tunelovaná data. Komunikující protějšky kontrolují zdrojovou \texttt{IP} adresu označených \texttt{ICMP} zpráv, aby nebylo triviálně možné injektovat komunikaci do tunelu. Těchto dodatečných pět bajtů je potřebných pro správnou funkci tunelu a předávání servisních zpráv. Jejich rozmístění je vyznačeno na diagramu \ref{diag:icmp-packet-tunel}.
	...
	\begin{bytefield}[bitwidth=1em]{32}
	    ...
	    \bitbox{16}{Identifikátor} & \bitbox{16}{Sekvenční číslo}
	    \\
	    \colorbitbox{lightyellow}{32}{Značka zprávy tunelu}
	    \\
	    \colorbitbox{lightyellow}{8}{Typ zprávy} & \bitbox[lr]{24}{}
	    \\
	    \bitbox[lbr]{32}{Data}
	\end{bytefield}
    ...
    V jednu chvíli server obsluhuje pouze jednoho klienta a požadavky o spojení jiných klientů ignoruje. Situace, kdy se klient připojuje k volnému server je zachycena na diagramu \ref{diag:seq-icmp}.
...
\begin{figure}[h]
 \centering
  ...
  \begin{tikzpicture}[node distance=5cm,auto,>=stealth']
    ...
    \node[below of=Klient, node distance=6cm] (client_ground) {};
    %
    \draw (Klient) -- (client_ground);
    \draw (Server) -- (server_ground);
    \draw[->] ($(Klient)!0.15!(client_ground)$) -- node[above,scale=1,midway]{Požadavek o spojení} ($(Server)!0.15!(server_ground)$);
    \draw[<-] ($(Klient)!0.20!(client_ground)$) -- node[below,scale=1,midway]{Přijetí požadavku} ($(Server)!0.20!(server_ground)$);
    \draw[->] ($(Klient)!0.51!(client_ground)$) -- node[above,scale=1,midway]{Odeslání \uv{NAT} paketů}($(Server)!0.51!(server_ground)$);
    \draw[->] ($(Klient)!0.54!(client_ground)$) -- 						 ($(Server)!0.54!(server_ground)$);
    \draw[->] ($(Klient)!0.57!(client_ground)$) -- 						 ($(Server)!0.57!(server_ground)$);
    \draw[->] ($(Klient)!0.60!(client_ground)$) -- 						 ($(Server)!0.60!(server_ground)$);
   
    \draw[->] ($(Klient)!0.80!(client_ground)$) -- node[above,scale=1,midway]{Obousměrná komunikace} ($(Server)!0.80!(server_ground)$);
    \draw[<-] ($(Klient)!0.85!(client_ground)$) -- 						 ($(Server)!0.85!(server_ground)$);
\end{tikzpicture}
